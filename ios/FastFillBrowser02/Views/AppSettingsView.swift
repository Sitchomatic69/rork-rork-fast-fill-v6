...
struct AppSettingsView: View {
    ...
    private var cycleProfileSection: some View {
        Section {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Generates a brand-new iOS device persona (UA, screen, GPU, audio, locale, timezone) and wipes every byte of WebKit state — cookies, cache, local storage, IndexedDB, service workers — across the default and all Quad sessions. The app cold-restarts under the new identity. Saved credentials are preserved.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
        ...
        .confirmationDialog(
            ...
            Text("This wipes all WebKit state and restarts the app. Saved credentials are kept.")
        }
    }
    ...
    private func runCycle() {
        ...
        Task {
            ...
            // If coldRestart returned (e.g. simulator), at least refresh the UI.
            await MainActor.run {
        ...
        Section {
            ...
            Button {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    HStack(spacing: 12) {
                        Label(localeDisplay(currentProfile.locale), systemImage: "globe")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
            ...
            Picker(selection: Binding(
                ...
                ForEach(FingerprintService.locales.filter { !$0.id.isEmpty }) { loc in
                    Text(loc.label).tag(loc.id)
                }
            } label: {
            ...
            Picker(selection: Binding(
                ...
                ForEach(FingerprintService.timezones.filter { !$0.id.isEmpty }) { tz in
    ...
    private func localeDisplay(_ locale: String) -> String {
        FingerprintService.locales.first(where: { $0.id == locale })?.label ?? locale
    }
}
